...
install()
{
	...
	dir.size = cur_path_inode->di_size / (DIRSIZ + 2);	//目录项的个数，即当前目录大小，每个目录项大小为16
	//初始化fcb中的name以及i节点，0是不用的i节点
	//循环遍历当前目录的所有目录项
	//将每个目录项的名称初始化为空格字符串，长度为14个空格
	// 将每个目录项的i节点索引号初始化为0，表示该目录项当前未被使用。
	for (i = 0; i < DIRNUM; i++)
	{
	...
	int x = dir.size / (BLOCKSIZ / (DIRSIZ + 2));  //计算存放目录项需要的磁盘块
	//根目录下的fcb有 没有用超过一个磁盘块存储，也就是除了0磁盘块，有 没有用别的磁盘块存储
	//保证目录占满了整个磁盘
	for (i = 0; i < x; i++)
	{
		...
		//根据当前目录的i节点(cur_path_inode)中的磁盘块块号(di_addr)，计算目标磁盘块的位置，并将文件指针(fd)移动到该位置准备读取数据
		fseek(fd, DATASTART + BLOCKSIZ * cur_path_inode->di_addr[i], SEEK_SET);

		//每一次都读32个目录项即一个磁盘块，保证全部读入
