...
*					2. 输出信息完善
					...
					5. 登陆时如果不是主目录的过程修改
					6. 格式化fmt输入后的处理错误
					7. fmt 中增加 先跳转到根目录 然后释放cur_path_inode
					8. creat函数参数修改带来的变化 ***
					9. 修改打开文件模块中的逻辑与判断 以及参数传递***
					10. 增加变量int pos_in_sysofile;用于文件读写和打开中
*/
#include <stdio.h>
#include "filesys.h"
#include <stdlib.h>
...
//将读入内存的i节点存入桶中，哈希函数dino % NHINO，一共512个i节点，NHINO=128
//读入桶中即可直接调用，无需再进行磁盘中的IO操作
...
void main()
{
	...
	do {
		...
		/**************************18rmdir*************************/
			...
			char temp_dir_name[DIRSIZ];//文件名长度14个字节 
