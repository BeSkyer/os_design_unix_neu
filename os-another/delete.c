...
int rmdir(int user_id, char* filename)
{
	for (int i = 0; i < dir.size; i++) {
		if (dir.direct[i].d_ino != 0)
		{
			...
			struct inode* inode;
			//获取磁盘i节点位置
			dinodeid = namei(dir.direct[i].d_name);
			//读取到内存
			if (dinodeid != NULL)
				inode = iget(dinodeid);
			else
			...
			if ((inode->di_mode & DIDIR) != 0 || (inode->di_mode & DIFILE) != 0)
			{
				...
				if ((inode->di_mode & DIDIR) != 0)
				{
					...
					//跳转到要删除的目录中
					//判断要删除的目录是否为空
					chdir(ROOT, dir.direct[i].d_name);
					//大于2就证明存在子目录，因为2用于存放索引指针
					if (dir.size > 2 && (strcmp(dir.direct[i].d_name, ".") != 0 && strcmp(dir.direct[i].d_name, "..") != 0))
					{
						rmdir(user_id, dir.direct[i].d_name);
						chdir(ROOT, "..");                    //跳回去
					}
					//如果不存在，先跳回去
					else { chdir(ROOT, ".."); }
